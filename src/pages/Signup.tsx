...
const Signup = () => {
    ...
    const [user,setUser] = useState<Inputs>(localStorage.getItem("user")?JSON.parse(localStorage.getItem("user") as string):{});
  ...
  return (
    ...
    <h3 className="text-2xl font-semibold text-blue-600">Create an account</h3>
    {isExist&& <h3 className="text-2xl font-semibold bg-red-300/[0.4] px-4 py-2 rounded-[14px] text-red-600"> user already exist try to login</h3>}
 ...
 <div className=" relative ">
      ...
      <div className=" relative ">
        <label
          htmlFor="text"
          className={`absolute top-[-16px] left-4 text-[18px] bg-white px-3 ${
            errors.age ? "text-red-500" : "text-green-600"
          }`}
        >
          Age
        </label>
        <input
          ...
          className={`border rounded-[10px] p-2 w-full ${
            errors.age ? "border-red-500" : "border-green-700"
          }`}
        />
        {errors.age && (
      ...
      <Link to="/login" className="text-blue-600 text-center cursor-pointer">You already have an account? Login</Link>
    </form>
  </div>
  )
}
